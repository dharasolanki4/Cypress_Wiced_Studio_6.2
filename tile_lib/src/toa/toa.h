...
/** \defgroup TOA
TOA is a transport using 2 GATT characteristics.<br>
...
Support for either version is discovered through Characteristic discovery (as these use different BLE characteristics).<br>

The main difference between the 2 are as follows:<br>
- Community Find:
...
- TOA Client:
  ...
  - There is no mandatory TOA feature and the TOA Client shall support that any TOA feature is not supported (like @ref TMD or @ref SONG for instance).
...
enum TOA_RSP
{
  ...
  ----------------------- | ------------- | ------------------------------------------ | ------------------- | -------------------------------
  1 Byte                  |  1 Byte       | 3 Bytes, check @ref TOA_SUPPORTED_FEATURES | 4 Bytes             | Varies, Reserved for Futur Use
  */

  TOA_RSP_TOFU_CTL      = 0x02, /**< see @ref TOFU_CTL_RSP */
...
enum TOA_CMD
{
  TOA_CMD_TOFU_CTL      = 0x01, /**< see @ref TOFU_CTL_CMD */
...
bit  | Feature
 --- | --------
bit 0| @ref TOFU : Tile Over the air Firmware Upgrade
bit 1| BDADDR : Tile BDADDR Access
bit 2| @ref TDT : Tile Double Tap
bit 3| @ref SONG : Tile Song Feature
...
bit 1| @ref TKA : Tile Keep Alive Feature
bit 2| @ref TDG : Tile Diagnostic Access
bit 3| @ref TMD : Tile Mode Control
bit 4| @ref TCU : Tile Connection Update Control
bit 5| @ref TIME : Tile Time-keeper
bit 6| @ref TEST : Tile TEST Feature
bit 7| TMA

BYTE 2:<br>
bit  | Feature
---- | -------
bit 0| @ref TFC : Tile Flow Control - DEPRECATED
bit 1| @ref TPS : Tile Programmable Songs
bit 2| @ref TMF : Tile Manufacturing
bit 3| @ref TLIL : Tile Lock-If-Lost
bit 4| @ref TEF : Tile Experimental Framework
bit 5| @ref TRM : Tile RSSI Monitoring
bit 6| @ref TPC : Tile Power Control
bit 7| @ref TSQ : Tile Song Quality
 */
...
bit  | Feature
---- | -------
bit 0| @ref TDE : Can receive TDE events
bit 1| RFU
bit 2| RFU
bit 3| RFU
bit 4| RFU
